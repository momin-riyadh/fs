...
function SidebarItem({ item }: { item: NavItem }) {
  ...
  return (
    <details className="group rounded-2xl border border-black/10 bg-white/70 px-3 py-2">
      <summary className="flex cursor-pointer list-none items-center justify-between px-1 py-2 text-sm font-semibold text-[color:var(--ink)]">
        {item.label}
    ...
    <div className="min-h-screen bg-transparent text-[color:var(--ink)]">
      <header className="sticky top-0 z-40 border-b border-black/10 bg-white/70 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <Link href="/" className="flex items-center gap-3">
            <span className="grid h-10 w-10 place-items-center rounded-2xl bg-[color:var(--gold)] text-sm font-semibold text-[color:var(--ink)] shadow-[0_12px_25px_rgba(244,162,89,0.4)]">
              CD
            </span>
          ...
          <details className="group relative">
            <summary className="flex cursor-pointer list-none items-center gap-3 rounded-full border border-black/10 bg-white/80 px-3 py-2 text-sm font-semibold text-black/70 shadow-sm transition hover:border-black/30">
              <span className="text-xs uppercase tracking-[0.2em] text-black/50">
                Momin
              </span>
              <span className="grid h-9 w-9 place-items-center rounded-full bg-[color:var(--sage)] text-sm font-semibold text-white">
                MO
              </span>
            </summary>
            <div className="absolute right-0 mt-3 w-44 rounded-2xl border border-black/10 bg-white/95 p-2 text-sm shadow-[0_18px_40px_rgba(0,0,0,0.12)]">
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-6 py-8 md:grid-cols-[240px_1fr]">
        <aside className="space-y-4">
          <div className="rounded-3xl border border-black/10 bg-white/80 p-4 shadow-[0_20px_45px_var(--shadow)] backdrop-blur">
